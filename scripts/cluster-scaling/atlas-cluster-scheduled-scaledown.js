/**
 * @file atlas-cluster-scheduled-scaledown.js
 * @description MongoDB Atlas Scheduled Trigger — Automated Cluster Scale-Down Workflow
 *
 * Workflow:
 *   1. Retrieve current cluster configuration via Atlas Administration API
 *   2. Validate active cluster tier against the expected scale-down source tier
 *   3. Execute conditional scale-down if criteria are met
 ...
 * Schedule      : Configure in Atlas App Services UI (e.g., daily at 22:00 IST)
 * Target Action : M80 General → M50 General (off-peak cost optimization)
 *
 * @author       Madan U
...
const SCALING_POLICY = {
  sourceTier:      "M80",   // Trigger scale-down only when cluster is at this tier
  targetTier:      "M50",   // Desired tier after scale-down
  maxInstanceSize: "M80",   // AutoScaling upper bound (allows Atlas to scale back up)
  diskIOPS:        8000,
  diskSizeGB:      750,
 ...
 * reversing the manual scale-down triggered by this workflow.
 */
 ...
 * @param {string} instanceSize - Target Atlas instance size (e.g., "M50")
 */
 ...
 * @param {string} targetTier        - Instance size to scale down to
 */
...
// ─────────────────────────────────────────────
// Core: Execute Cluster Scale-Down
// ─────────────────────────────────────────────

/**
 * Submits a PATCH request to the Atlas Administration API to scale down the cluster.
 ...
 */
const executeClusterScaleDown = async (client, currentTier, replicationSpecId) => {
  const { targetTier } = SCALING_POLICY;
  const url = `${ATLAS_API_BASE}/groups/${CLUSTER_CONFIG.projectId}/clusters/${CLUSTER_CONFIG.clusterName}`;

  console.log(`[INFO] Initiating scale-down: ${currentTier} → ${targetTier}`);
  ...
  console.log(`[INFO] Scale-down initiated successfully.`);
 ...
 * Orchestrates the full scale-down workflow:
 *   fetch config → validate tier → conditional scale-down → log outcome
 */
exports = async function () {
  console.log("─────────────────────────────────────────────────");
  console.log("[START] Atlas Cluster Scale-Down Workflow");
  ...
  try {
    ...
    if (clusterTier !== SCALING_POLICY.sourceTier) {
      ...
      console.log("[END]  Workflow exited — scale-down condition not met.");
      return;
    }

    // Step 3: Execute scale-down
    await executeClusterScaleDown(client, clusterTier, replicationSpecId);

    console.log("[SUCCESS] Scale-down workflow completed.");
  ...
  console.log("─────────────────────────────────────────────────");
  console.log("[END] Atlas Cluster Scale-Down Workflow");
  console.log("─────────────────────────────────────────────────");
};
